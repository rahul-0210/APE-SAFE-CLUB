...
contract CoinFlip is Ownable{
    ...
    function cancel(uint _gameCount) public {
        GameStatics memory _gameStatics = gameStatics[_gameCount];
        require(msg.sender == _gameStatics.player1, "Not Authorized");
        require(_gameStatics.player2 == address(0), "Invalid Move");

        require(token.transfer(msg.sender, _gameStatics.betAmount), "Token Trasfer Failed");
        gameStatics[_gameCount].betAmount = 0;
        emit CancelFlipGame(msg.sender, _gameCount, 0);
    }

    function takeBet(uint _gameCount) public {
        GameStatics memory _gameStatics = gameStatics[_gameCount];
        require(_gameStatics.player2 == address(0), "Invalid Move");

        require(token.transferFrom(msg.sender, address(this), _gameStatics.betAmount), "Token Trasfer Failed");
        gameStatics[_gameCount].player2 = msg.sender;
